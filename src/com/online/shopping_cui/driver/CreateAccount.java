...
public class CreateAccount {
    ...
    public CreateAccount(Scanner scanner, HashMap<String, User> users) {
    ...
    public boolean createCustomerAccount() {
        ...
        do{
            ...
            if (loginID.equalsIgnoreCase("b")) { // If user wishes to go back...
                return false; // Go back up a level.
            } else if(loginID.trim().isEmpty() || users.containsKey(loginID)){ // While user-defined login ID is empty or exists within collection...
                System.err.println("Please choose another login ID."); // Prompt user for another login id.
            }
        }  while (loginID.trim().isEmpty() || users.containsKey(loginID)); // While user-defined login ID is empty or exists within collection...

        do {
            System.out.print("\tPassword*: ");
            password = scanner.nextLine();

            if (password.equalsIgnoreCase("b")) { // If user wishes to go back...
                return false; // Go back up a level.
            } else if (!Utilities.passIsSecure(password, 8)) { // If user-defined password is not secure enough...
    ...
    protected boolean createAdministratorAccount() {
        ...
        do{
            ...
            if (loginID.equalsIgnoreCase("b")) { // If user wishes to go back...
                return false; // Go back up a level.
            } else if(loginID.trim().isEmpty() || users.containsKey(loginID)){ // If user-defined login ID is empty or exists within collection...
                System.err.println("Please choose another login ID."); // Prompt user for another login id.
            }
        }  while (loginID.trim().isEmpty() || users.containsKey(loginID)); // While user-defined login ID is empty or exists within collection...
        ...
        do {
            ...
            if (password.equalsIgnoreCase("b")) { // If user wishes to go back...
                return false; // Go back up a level.
            } else if (!Utilities.passIsSecure(password, 14)) { // If user-defined password is not secure enough...
